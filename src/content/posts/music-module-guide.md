...
description: 拆开 Firefly 的音乐模块，看看它如何用一个全局音频实例连接侧栏与顶栏播放器，并处理歌单、进度、播放模式、歌词同步和站内切页。
image: random
...
博客里的音乐卡片最初看起来只是几个按钮：封面、歌名、进度条，再加上播放和切歌。真正把它放进 Firefly 后，问题很快就不只剩下“怎么让音频响起来”。

侧栏有一张播放器，顶栏也能打开一张播放器；页面切换时音乐不能突然从头开始，更不能两个播放器各放各的。歌词要跟着进度走，歌单接口偶尔失效时也得有退路。现在这套音乐模块，主要就是围绕这些细节搭起来的。
